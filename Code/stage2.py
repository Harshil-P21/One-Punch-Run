...
####################################################################### MAKING
class fishman(object):
    walkRight = [pygame.image.load('fish1r.png'),pygame.image.load('fish2r.png'),pygame.image.load('fish3r.png'),pygame.image.load('fish4r.png')]
    walkLeft = [pygame.image.load('fish1l.png'),pygame.image.load('fish2l.png'),pygame.image.load('fish3l.png'),pygame.image.load('fish4l.png')]
    ...
    def draw(self,win):
        if self.visible:
            ...
            pygame.draw.rect(win, (255,0,0), (self. hitbox[0]-24, self.hitbox[1]-10, 100, 10,))
            pygame.draw.rect(win, (0,255,0), (self. hitbox[0]-24, self.hitbox[1]-10, 50 - (5 *(10 - self.health)), 10,))  
            self.hitbox= (self.x + 40, self.y+10, 40, 80)
            #pygame.draw.rect(win, (255,0,0), self.hitbox, 2)
    def move(self):
        if self.vel > 0:
...
while run:
    ...
    if fish.visible == True:
        if man.hitbox[1] < fish.hitbox[1] + fish.hitbox[3] and man.hitbox[1] + man.hitbox[3] > fish.hitbox[1]:
            if man.hitbox[0] + man.hitbox[2] > fish.hitbox[0] and man.hitbox[0] < fish.hitbox[0] + fish.hitbox[2]:
                man.hit()
                score -= 5
    if shoot > 0:
        shoot +=1
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
    for bullet in bullets:        
        if bullet.y - bullet.radius < fish.hitbox[1] + fish.hitbox[3] and bullet.y +bullet.radius > fish.hitbox[1]:
            if bullet.x + bullet.radius > fish.hitbox[0] and bullet.x - bullet.radius < fish.hitbox[0] + fish.hitbox[2]:
                if fish.visible == True:
                    hitsound.play()
                    fish.hit()
                    score += 1
